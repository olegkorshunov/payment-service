...
class AsyncSQLAlchemyOptions(base.FactoryOptions):
    ...
    async def use_postgeneration_results(self, step: Any, instance: Any, results: Dict[str, Any]) -> None:
...
class AsyncStepBuilder(StepBuilder):
    async def build(self, parent_step: Optional[BuildStep] = None, force_sequence: Any = None) -> Any:
        ...
        await self.factory_meta.use_postgeneration_results(instance=instance, step=step, results=postgen_results)
        return instance
...
class AsyncSQLAlchemyFactory(factory.Factory):
    ...
    async def _get_or_create(
        ...
        async with session() as db_session:
            obj = (await db_session.execute(select(model_class).filter_by(*args, **key_fields))).scalars().one_or_none()
